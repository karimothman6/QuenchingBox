...
c = st.slider("Carbon (C)", 0.2, 0.32, 0.25, step=0.01)
si = st.slider("Silicon (Si)", 0.14, 0.55, 0.3, step=0.01)
mn = st.slider("Manganese (Mn)", 0.6, 1.8, 1.2, step=0.1)
p = st.slider("Phosphorus (P)", 0.006, 0.04, 0.02, step=0.001)
s = st.slider("Sulfur (S)", 0.009, 0.04, 0.02, step=0.001)
ni = st.slider("Nickel (Ni)", 0.01, 0.25, 0.1, step=0.01)
cr = st.slider("Chromium (Cr)", 0.026, 0.3, 0.15, step=0.01)
mo = st.slider("Molybdenum (Mo)", 0.007, 0.1, 0.05, step=0.01)
cu = st.slider("Copper (Cu)", 0.131, 0.7, 0.4, step=0.01)
v = st.slider("Vanadium (V)", 0.00024, 0.013, 0.005, step=0.001)
n = st.slider("Nitrogen (N)", 0.0056, 0.014, 0.01, step=0.001)
ce = st.slider("CE%", 0.35, 0.61, 0.5, step=0.01)
